...
-- dann return ein composed Subst von all diesen.
  ...
  foldr Subst.compose Subst.identity (map (getSubst) (findAllVars t1))
  )
  | otherwise = Nothing
  where
    -- gets the Substitution from t1.pos (which is a var) to t2.pos
  getSubst ::  Pos -> Subst
  getSubst matchedPosition =
    let grabVar = (selectAt t1 matchedPosition)
        grabTerm = (selectAt t2 matchedPosition)
    in getSubst' grabVar grabTerm
    where
      -- construct a Subst from a Variable and a Term.
      getSubst' :: Term -> Term -> Subst
      getSubst' (Var varname) term = single varname term
      getSubst' (Comb _ _) _ = error "no"
...
ace t1 t2  = let allCombs = (allPos t1) \\ (findAllVars t1) in
  ...
  where
    ...
    sameStructure (Var _) (Comb _ _) = error "fuck you"
    sameStructure (Comb _ _) (Var _) = error "fuck you"
    sameStructure x@(Var _) y@(Var _) = x==y
